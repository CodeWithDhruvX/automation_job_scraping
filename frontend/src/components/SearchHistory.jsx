...
export function SearchHistory({ history, onSelectHistory, onClearHistory, onToggleSaved }) {
    ...
    return (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm min-h-[520px] flex flex-col sticky top-24 transition-colors"
            style={{ maxHeight: 'calc(100vh - 8rem)' }}
        >
            {/* Tabs Header */}
            <div className="flex items-center px-4 pt-4 border-b border-slate-100 dark:border-slate-700">
                <button
                ...
                {filteredHistory.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center py-12 px-4">
                        <div className="bg-slate-100 dark:bg-slate-700 p-4 rounded-full mb-3">
                            {activeTab === 'saved' ? <Star size={32} className="text-slate-400 dark:text-slate-500" /> : <History size={32} className="text-slate-400 dark:text-slate-500" />}
                        </div>
                    ...
                    filteredHistory.map((item) => (
                        <div
                            ...
                            className="p-3 border border-slate-200 dark:border-slate-700 rounded-lg hover:border-blue-400 dark:hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 cursor-pointer transition-all group relative pr-8"
                        >
                            {/* Star Button */}
                            <button
                                onClick={(e) => {
                            ...
                            <div className="flex items-start justify-between gap-2 mb-2">
                                <div className="flex-1 min-w-0">
                                    <h4 className="font-semibold text-sm text-slate-800 dark:text-slate-200 truncate group-hover:text-blue-700 dark:group-hover:text-blue-400">
                            ...
                            <div className="flex flex-wrap gap-1">
                                {Object.entries(item.filters.sites).map(([site, active]) => (
                                    active && (
                                        <span
                                            key={site}
                                            className="px-2 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 rounded text-xs capitalize group-hover:bg-blue-100 dark:group-hover:bg-blue-900/40 group-hover:text-blue-700 dark:group-hover:text-blue-300"
                                        >
                                            {site.replace('_', ' ')}
                                        </span>
                                    )
            ...
            {/* Footer */}
            <div className="px-4 py-2 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 text-xs text-slate-500 dark:text-slate-400 text-center transition-colors">
                {activeTab === 'saved'
